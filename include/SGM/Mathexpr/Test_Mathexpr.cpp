...
static void intro()
{
	mdo
	<<	Title("Introduction") 
	<<	"sgm::Mathexpr supports some fundamental mathematical functions which operate "
	<<	"in \"compile time\", not in runtime."
	<<	empty_line;
}
...
{
	mdo 
	<<	Title("Power Functions with Integer Power")
	<<	Title("Real Number Base", 2)
	<<	Load_code_block("1/2_as_base_ex") << newl;
...
END_CODE_BLOCK_AND_LOAD(pow_with_int_base_ex)
	...
	mdo << Title("Exponential Function", 2);
...
SGM_SPECIFICATION_TEST(sgm::spec::Test_, Mathexpr, /**/)
{	::intro
,	::constexpr_pi
,	::constexpr_factorial
,	::constexpr_pow
