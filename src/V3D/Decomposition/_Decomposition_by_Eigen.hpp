...
class v3d::Eigen_Decomposition<T, ROWS, COLS, STOR, IS_REAL_SYMMETRIC>::_impl_t
{
private:
	using _Mat_t = Eigen::Matrix< T, static_cast<int>(ROWS), static_cast<int>(COLS), static_cast<int>(STOR) >;

	std::conditional_t< IS_REAL_SYMMETRIC, Eigen::SelfAdjointEigenSolver<_Mat_t>, Eigen::EigenSolver<_Mat_t> > _solver;
	bool _has_computed_eigenvectors = false;

	using _Elem_t = typename std::decay_t<decltype(_solver.eigenvectors())>::value_type;
...
public:
	...
	{
		_has_computed_eigenvectors = !sgm::is_Selected_Flag<flag::Value_Only, FS>::value;

		if constexpr(IS_REAL_SYMMETRIC)
			_solver.compute
			(	_Mat_implementor( std::forward<MAT>(m) )
			,	_has_computed_eigenvectors ? Eigen::ComputeEigenvectors : Eigen::EigenvaluesOnly
			);
		else
			_solver.compute(  _Mat_implementor( std::forward<MAT>(m) ), _has_computed_eigenvectors  );
	}

	auto size() const-> size_t{  return _solver.eigenvalues().size();  }
	decltype(auto) eigenval(size_t const idx) const{  return _solver.eigenvalues()( static_cast<int>(idx) );  }

	auto eigenvec(size_t const idx) const-> v3d::_MatrixAdaptor<_Elem_t, ROWS, 1, STOR>
	{
	...
	auto diagmat() const
	{
		...
		v3d::Matrix<_Elem_t, ROWS, COLS, STOR> res = v3d::Matrix<_Elem_t, ROWS, COLS, STOR>::Zero(dim, dim);
	...
	auto basemat() const-> v3d::_MatrixAdaptor<_Elem_t, ROWS, COLS, STOR>{  return _solver.eigenvectors();  }
};
//--------//--------//--------//--------//-------#//--------//--------//--------//--------//-------#//--------//--------


template<class T, size_t ROWS, size_t COLS, v3d::Storing_Order STOR>
class v3d::Singular_Value_Decomposition<T, ROWS, COLS, STOR>::_impl_t
{
private:
...
public:
	...
	void operator()(MAT &&m, FS &&fs)
	{
		...
		(	_Mat_implementor( std::forward<MAT>(m) ), _bit_flag<FS>()
		);
		...
		if constexpr(sgm::is_Selected_Flag<flag::Value_Only, FS>::value)
			_clear();		// clear _U and _V but leave _values unchanged. 
		else
			_calc_UV( svd, std::forward<FS>(fs) );
	}
	...
	auto diagmat() const-> DynamicMat<T, STOR>
	{
		...
		=	Umat().size() != 0 && Vmat().size() != 0 && is_Square_Matrix(Umat()) && is_Square_Matrix(Vmat());
...
private:
	...
	static unsigned constexpr _bit_flag()
	{
		...
		{
			unsigned constexpr
				MODE = sgm::is_Selected_Flag<flag::FullMat, FS>::value ? 1 : 0,
				UB 
				=	sgm::is_Selected_Flag<flag::VMat_Only, FS>::value ? 0
				:	MODE == 1 ? Eigen::ComputeFullU
				:	/* otherwise */ Eigen::ComputeThinU,
				VB
				=	sgm::is_Selected_Flag<flag::UMat_Only, FS>::value ? 0
	...
	void _clear(M &m = sgm::none)
	{
		if constexpr(sgm::is_None_v<M>)
			_clear(_U),  _clear(_V);
		else
			m = std::decay_t<decltype(m)>{};
	}
	...
	{
		if constexpr(sgm::is_Selected_Flag<flag::VMat_Only, FS>::value)
			_clear(_U);
		else
			_U = _MA_t(svd.matrixU());


		if constexpr(sgm::is_Selected_Flag<flag::UMat_Only, FS>::value)
			_clear(_V);
		else
			_V = _MA_t(svd.matrixV());


		if constexpr(sgm::has_Satisfying_Flag<flag::is_Truncated, FS>::value)
			sgm::Satisfying_Flag<flag::is_Truncated>(fs).cut(_values, _U, _V);
	}
};
//========//========//========//========//=======#//========//========//========//========//=======#//========//========
...
template<>
class v3d::_Least_Square_Solution_Helper<v3d::Solving_Mode::SVD>
{
...
template<>
class v3d::_Least_Square_Solution_Helper<v3d::Solving_Mode::QR>
{
	friend struct Least_Square_Problem;
