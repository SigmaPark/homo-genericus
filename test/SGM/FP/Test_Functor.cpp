...
{
	{
		...
		H2U_ASSERT( ftr(x, 3, s) == 10*(x + 3) - s.value() );
	}
	{
		int x = 5;
		Specimen s{20};
		...
		H2U_ASSERT( ftr(3) == 10*(x + 3) - s.value() );		
	}
}
...
{
	{
		...
		H2U_ASSERT( composed_ftr(4, 2) == -10*(10*4 + 2) );
	}
	{
...
void Test::Basel_Problem()
{
	...
	H2U_ASSERT( std::abs(res - answer) < 1e-4 );
}
//========//========//========//========//=======#//========//========//========//========//=======#


H2U_HOW2USE_TESTS(sgm::test::Test_, Functor, /**/)
{	::Test::Lambda
,	::Test::Functor
,	::Test::Blank
,	::Test::Composition
