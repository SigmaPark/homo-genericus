...
void Circular_Queue_Contents::Push_and_Pop2()
{
	...
	SGM_H2U_ASSERT(qu.size() == 3 && qu.is_full() && qu.front() == 5 && qu.back() == 9);

	qu.pop();

	SGM_H2U_ASSERT(qu.size() == 2 && qu.front() == 7 && qu.back() == 9);
}
...
void Circular_Queue_Contents::Move_Construction()
{
	...
	SGM_H2U_ASSERT(qu.is_empty() && qu.max_size() == 0);
}
